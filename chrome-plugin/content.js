...
async function createAgentButton() {
  const headerWrapper = await findHeaderElement();
  ...
  if (headerWrapper) {
    ...
    iframe.src = 'http://localhost:5173?wsId=' + (isDesk ? '2' : '1');
    ...
    button.className = !isDesk ? 'btn chat-with-us' : 'nucleus-button nucleus-button--secondary';
    button.type = 'button';
